...
def train_sample(sample, compute_metrics=False):
    ...
    save_image(t_umaps[-1].float(),'test_umap.png')
...
def get_dis_loss(preds_S, preds_T,student_channels, teacher_channels, lambda_mgd=0.15, mask=None):
    ...
    # threshold for umaps
    # Using 050 as a base thresholding
    thresh = 0.50
    ...
    if mask is not None:
        ...
        # print("dialation size: ",(256//H)//4, H, W) 
    ...
    masked_feat = torch.mul(preds_S, mat1)
...
def sampling(mat,dilation_factor=10):
    ...
    # print("number of masked points: ",zeros_count )
    
    for i in range(mat.size()[0]): 
        if(zeros_count[i]//2 !=0):
            non_zero_loctions = torch.nonzero(edges[i,:,:,:]==0)
            random_indices = torch.randperm(len(non_zero_loctions))[:zeros_count[i]//2]
            # print("random indices: ",random_indices.size())
            selected_tensors = [non_zero_loctions[i] for i in random_indices]
            # print("selected tensors: ",selected_tensors.size())
            selected_tensors = torch.stack(selected_tensors,dim=0)
            # print("selected tensors: ",selected_tensors.size())
            # print("selected tensors: ",selected_tensors.size())
            mat1[i,selected_tensors[:,0],selected_tensors[:,1],selected_tensors[:,2]] = 0 

    # zeros_count = torch.sum(mat1 == 0, dim=(2,3))
    # print("number of masked points after dialation inclusion: ",torch.sum(mat1 == 0, dim=(2,3)))
    # print("After dilation: ",mat1)


    # Random sampling from the unmasked areas
    
    for i in range(mat1.size()[0]): 
        if(zeros_count[i]//2 !=0):
            non_zero_loctions = torch.nonzero(mat1[i,:,:,:]==1)
            random_indices = torch.randperm(len(non_zero_loctions))[:zeros_count[i]//2]
            # print("random indices: ",random_indices.size())
            selected_tensors = [non_zero_loctions[i] for i in random_indices]
            # print("selected tensors: ",selected_tensors.size())
    ...
    # print("number of masked points after new samples: ",torch.sum(mat1 == 0, dim=(2,3)))
    # print("After new samples: ",mat1)

    # random_points = __get_coords(gt * (1. - edges))
    # edge_points = __get_coords(gt * edges)

    # # if edge points exist
    # if edge_points.shape[0]>0:

    #     # Check tot num of edge points
    #     cond = edges.sum()//2 -  num_sample_inout//2 < 0
    #     tot= (num_sample_inout - int(edges.sum())//2, int(edges.sum())//2) if cond else \
    #          (num_sample_inout//2, num_sample_inout//2)
...
def get_boundaries(disp, th=1., dilation=10):
    ...
    # edges_y = torch.logical_or(F.pad(torch.abs(disp[1:, :] - disp[:-1, :]) > th, ((1, 0), (0, 0))),
    #                         F.pad(torch.abs(disp[:-1, :] - disp[1:, :]) > th, ((0, 1), (0, 0))))
    ...
    if dilation > 0:
        ...
        dilation_kernel = torch.ones(1, 1, *kernel_size).to(disp.device)

        disp = disp.float()
        disp = 1-disp

        # print("disp: ",disp.size(),disp.type())
        # print("kernel: ",dilation_kernel.size(),dilation_kernel.type())
        # Perform dilation using conv2d
        edges = F.conv2d(disp, dilation_kernel, padding=((kernel_size[0] -1)// 2, (kernel_size[1]-1) // 2))
        
        edges = (edges > 0).float()
        
        edges = 1 - edges
        #edges = cv2.dilate(disp, kernel, iterations=1)
        #edges = F.conv2d(disp, kernel.unsqueeze(0).unsqueeze(0).expand(3, 1, -1, -1).contiguous(), padding=dilation//2)
