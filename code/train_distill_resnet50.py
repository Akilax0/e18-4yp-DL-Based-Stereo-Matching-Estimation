...
def train_sample(sample, gen1,gen2,gen3, compute_metrics=False):
    ...
    disp_ests,s_ll,s_rl,s_volume,_,_,_ = model(imgL, imgR)
    ...
    with torch.no_grad():
        ...
        t_disp_ests,t_ll,t_rl,t_volume,t_umaps = t_model(imgL,imgR)
        # tumap size 1/4
    ...
    # print("Volume align student , teacher: ",s_volume.size(),t_volume.size())
    # s_volume: 10,8,48,64,128  , t_volume : 10, 46, 48, 64, 128
    s_volume = align(s_volume,s_volume.size()[1],t_volume.size()[1])
    ...
    # cvolume_loss = get_dis_loss_3D(preds_S=s_volume,preds_T=t_volume,student_channels=s_volume.size()[1],teacher_channels=t_volume.size()[1],lambda_mgd=lambda_mgd) 
    
    # print("Loss feat vs volume: ", feat_loss, 0.001*feat_loss,0.01*cvolume_loss, loss)
...
def get_dis_loss(preds_S, preds_T,student_channels, teacher_channels, lambda_mgd=0.15, mask=None, generation=None):
    ...
    # # save_image(mat.float(),'resnet_50_exp/0_test_mask.png')
