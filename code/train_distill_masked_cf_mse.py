...
def train_sample(sample, compute_metrics=False):
    ...
    feat_loss = feat_loss + mse_loss(s_ll[0], t_ll[1])  
    feat_loss = feat_loss + mse_loss(s_ll[1], t_ll[2])  
    feat_loss = feat_loss + mse_loss(s_ll[2], t_ll[3])  
    feat_loss = feat_loss + mse_loss(s_ll[3], t_ll[4])  

    feat_loss = feat_loss + mse_loss(s_rl[0], t_rl[1])  
    feat_loss = feat_loss + mse_loss(s_rl[1], t_rl[2])  
    feat_loss = feat_loss + mse_loss(s_rl[2], t_rl[3])  
    feat_loss = feat_loss + mse_loss(s_rl[3], t_rl[4])  
...
def mse_loss(student,teacher):
    # print("student teacher: ",student.size(),teacher.size())
