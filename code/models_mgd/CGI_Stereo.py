...
class CGI_Stereo(nn.Module):
    ...
    def forward(self, left, right, teacher):
        
        # included in the teacher 
        # dictionary t_feat,t_cvolume,t_conv4,t_conv8

        '''
        
        Hyperparameter setting for Knowledge distillation
        ...
        '''
        Aligning student features to teacher features 
        Feature map 1/4 = feature_left[0]
        cost volume = volume
        conv4 = cost
        conv8 = conv8
        ...
        if self.training :
            ...
            s_feat = align(features_left[0],features_left[0].size()[1],teacher['t_feat'].size()[1])
            feat_loss,s_feat = get_dis_loss(preds_S=s_feat, preds_T=teacher['t_feat'], student_channels=s_feat.size()[1] , teacher_channels=teacher['t_feat'].size()[1], lambda_mgd=teacher['lambda_mgd'])
            s_feat_red = reducer(s_feat,features_left[0].size()[1])
            # print("Feature original student, generated, reduced:  ",features_left[0].size(),s_feat.size(),s_feat_red.size())
            features_left[0] = s_feat_red
        ...
        # # Used for distillation
        # s_cvolume = align(volume,volume.size()[1],t_cvolume.size()[1])
        # cvolume_loss,s_cvolume = get_dis_loss_3D(preds_S=s_cvolume,preds_T=t_cvolume,student_channels=s_cvolume.size()[1],teacher_channels=t_cvolume.size()[1],lambda_mgd=lambda_mgd) 
        # s_cvolume_red = reducer(s_cvolume,volume.size()[1])
        # # print("Volume original student, generated, reduced:  ",volume.size(),s_cvolume.size(),s_cvolume_red.size())
        # volume = s_cvolume_red
        # # print("Masked cvolume loss:  ",cvolume_loss)

        cost,conv8= self.hourglass_fusion(volume, features_left)

        # # Used for distillation
        # s_conv4 = align(cost,cost.size()[1],t_conv4.size()[1])
        # s_conv8 = align(conv8,conv8.size()[1],t_conv8.size()[1])
        # conv4_loss,s_conv4 = get_dis_loss_3D(preds_S=s_conv4,preds_T=t_conv4,student_channels=s_conv4.size()[1],teacher_channels=t_conv4.size()[1],lambda_mgd=lambda_mgd) 
        # conv8_loss,s_conv8 = get_dis_loss_3D(preds_S=s_conv8,preds_T=t_conv8,student_channels=s_conv8.size()[1],teacher_channels=t_conv8.size()[1],lambda_mgd=lambda_mgd) 
        # s_conv4_red = reducer(s_conv4,cost.size()[1])
        # # print("Conv4 original student, generated, reduced:  ",cost.size(),s_conv4.size(),s_conv4_red.size())
        # s_conv8_red = reducer(s_conv8,conv8.size()[1])
        # # print("Conv8 original student, generated, reduced:  ",conv8.size(),s_conv8.size(),s_conv8_red.size())
        # cost = s_conv4_red
        # conv8 = s_conv8_red
        # # print("Masked conv4 loss:  ",conv4_loss)
        # # print("Masked conv8 loss:  ",conv8_loss)
        ...
        if self.training:
            kd_loss = teacher['kd_loss']
            feat_loss = teacher['feat_loss'] 
            cvolume_loss = teacher['cvolume_loss'] 
            conv4_loss = teacher['conv4_loss'] 
            conv8_loss = teacher['conv8_loss'] 

            lambda_feat = teacher['lambda_feat']
            lambda_cvolume = teacher['lambda_cvolume']
            lambda_conv4 = teacher['lambda_conv4']
            lambda_conv8 = teacher['lambda_conv8']
            ...
            return [pred_up*4, pred.squeeze(1)*4], kd_loss
