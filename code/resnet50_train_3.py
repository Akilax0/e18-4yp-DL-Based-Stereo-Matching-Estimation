...
# train one sample
def train_sample(sample, gen1,gen2,gen3, st, epoch, compute_metrics=False):
    ...
    # print("tumaps: ",len(t_down_umaps))
    # # # if epoch == 0:
    # #     # save_image(t_umaps[-1].float(),'resnet50_umaps/'+str(time.time()-st)+'_test_umap.png')
    ...
    # feat_loss = feat_loss + get_dis_loss(st,s_ll[0], t_ll[0],student_channels=s_ll[0].size()[1], teacher_channels=4, lambda_mgd=lambda_mgd, mask = t_down_umaps[0], generation = gen1)  
